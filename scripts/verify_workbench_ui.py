...
try:
    ...
    with connect(target["webSocketDebuggerUrl"], max_size=20_000_000, proxy=None) as ws:
        ...
        wait("document.querySelectorAll('[data-analysis-preset]').length >= 8 && !document.getElementById('analyst-ask').disabled")
        assert js("document.getElementById('analyst-summary').textContent.includes('185.4')")
        js("document.querySelector('[data-analysis-preset=\"0\"]').click()")
        wait("!document.getElementById('analyst-result').hidden && !document.getElementById('analyst-ask').disabled")
        assert js("document.getElementById('analyst-result').textContent.includes('17,228')")
        assert js("document.querySelectorAll('.analyst-bar').length === 4")
        shot("analyst-outcomes.png")
        js("document.querySelector('[data-analysis-preset=\"2\"]').click()")
        wait("!document.getElementById('analyst-ask').disabled")
        assert js("document.getElementById('analyst-result').textContent.includes('9,732')")
        shot("analyst-customer-response.png")
        if args.local_assistant:
            js("document.getElementById('analyst-question').value='Show denied cases only'; document.getElementById('analyst-question-form').requestSubmit()")
            wait("!document.getElementById('analyst-ask').disabled")
            assert js("document.getElementById('analyst-result').textContent.includes('3,747 of 31,500')")
            js("document.getElementById('analyst-question').value='Predict the best solution'; document.getElementById('analyst-question-form').requestSubmit()")
            wait("!document.getElementById('analyst-ask').disabled")
            assert js("document.getElementById('analyst-status').textContent.includes('prediction model')")
        js("document.querySelectorAll('[data-analysis-preset]')[8].click()")
        wait("!document.getElementById('analyst-ask').disabled")
        assert js("document.querySelectorAll('.analyst-line').length === 1")
        call("Page.reload")
        wait("document.querySelectorAll('[data-analysis-history]').length >= 3")
        js("document.querySelector('[data-analysis-history]').click()")
        assert js("!document.getElementById('analyst-result').hidden")
